 ...
 * @description Reusable interaction status selector component with color-coded status badges.
...
export const StatusSelector: React.FC<StatusSelectorProps> = React.memo(({
  ...
  const initialIndex = useMemo(() => {
    const idx = STATUS_OPTIONS.findIndex((opt) => opt.label === value);
    return idx >= 0 ? idx : 0;
  }, [value]);

  const handleSelect = (statusVal: StatusType) => {
    onChange(statusVal);
    setIsOpen(false);
  ...
  const {
    ...
    itemCount: STATUS_OPTIONS.length,
    isOpen,
    onSelect: (idx) => {
    ...
    <div className="relative" ref={containerRef} onKeyDown={handleKeyDown}>
      ...
      <button
        ...
        className={`w-full px-3.5 py-2.5 text-sm font-medium bg-white dark:bg-slate-800 border rounded-xl shadow-2xs focus:outline-none cursor-pointer flex items-center justify-between text-left transition-colors ${
          !value ? 'text-slate-400 dark:text-slate-500' : 'text-slate-800 dark:text-slate-100 font-bold'
        } ${
          error
      ...
      >
        <span className="truncate flex items-center gap-2">
          {selectedOption ? (
            <>
              <span className={`w-2.5 h-2.5 rounded-full ${selectedOption.dotColor} shrink-0`} />
