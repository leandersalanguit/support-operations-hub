 ...
 * Uses a semantic, accessible native HTML select element when multiple numbers exist.
 */

import React, { useState, useRef } from 'react';
import { Phone, Info, ChevronDown } from 'lucide-react';
...
export const PhoneNumberField: React.FC<PhoneNumberFieldProps> = React.memo(({
  ...
  // Render a clean native select matching ProductSelector/ClassificationSelector/StatusSelector!
      ...
      <div className="relative">
        <div className="relative">
          <select
            ...
            className={`w-full pl-9 pr-9 py-2.5 text-sm font-semibold font-mono bg-white dark:bg-slate-800 border rounded-xl shadow-2xs focus:outline-none cursor-pointer appearance-none transition-colors ${
              !value ? 'text-slate-400 dark:text-slate-500 font-sans' : 'text-slate-800 dark:text-slate-100'
            } ${
              error
          ...
          >
            <option value="" disabled className="font-sans bg-white dark:bg-slate-800 text-slate-400 dark:text-slate-500">
              -- Select Phone Number ({clientPhoneNumbers.length} saved) --
            </option>
            {clientPhoneNumbers.map((num) => (
              <option key={num} value={num} className="font-mono bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100">
                📞 {num}
              </option>
            ))}
            <option value="__add_new__" className="font-sans bg-white dark:bg-slate-800 text-fotoblue-600 dark:text-fotoblue-400 font-bold">
              + Enter a new phone number...
            </option>
          </select>
          <Phone className="w-3.5 h-3.5 text-fotoblue-600 dark:text-fotoblue-400 pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2" />
